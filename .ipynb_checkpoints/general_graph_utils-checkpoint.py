...
###################################
# general-graph-utils.py
# This library allows the user to create linear framework graphs (finite, directed graphs with no self-loops and labeled edges), as well as useful data structures containing structural and topological information about the graph. This software was developed using the NetworkX (https://networkx.org/documentation/stable/index.html) software package.

# The library also allows a user to obtain thermodynamic information about the graph (e.g. the thermodynamic force of a cycle) and calculate the Steinberg signature.
###################################

###################################
# Creating a linear framework graph
###################################

def random_graph_SC(m=3, n=20):
    """
    Randomly generates a linear framework graph -- a finite, directed graph with no self-loops -- that is strongly connected and fully reversible. The size of the graph is randomly determined from range (m, n), and the edges are added by randomly selecting a pair of nodes in G.
    ...
    # determine the number of nodes in G
    G_size = np.random.choice(np.arange(m,n), size=1)
...
###################################
# Obtaining cycle information and calculating thermodynamic force (needs work)
###################################
